...
void OLED_WR_Byte(unsigned char dat,unsigned char cmd)
{
	if(cmd) 
	  {
       OLED_WriteData(dat); //写入数据
	  }
	else {
       OLED_WriteCommand(dat); //写入命令
	}
} 
/**
  * @brief  OLED设置光标位置
...
void OLED_DrawGIF(unsigned char x0, unsigned char y0, unsigned char x1, 
                 unsigned char y1, unsigned char k, unsigned int m, 
                 unsigned char GIF[][m])
{
	...
	for (i = 0; i < k; i++) // 从第一帧开始画
	{
		...
		{
			OLED_SetCursor(y, x0); // 设置光标位置（注意参数顺序：先Y后X）
   			
			for(x = x0; x <= x1; x++) // 画x1 - x0 + 1列
	    	{
	    		OLED_WR_Byte(GIF[i][j++], OLED_DATA); // 画图片的点
	    	}
		}
		Delay_ms(80); // 添加延时控制动画速度
	}
}